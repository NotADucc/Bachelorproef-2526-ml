...
class LoggerWrapper:
    def __init__(self, formatter: logging.Formatter, name: str):
...
def setup_custom_logger(formatter: logging.Formatter, name: str) -> LoggerWrapper:
    """ Set up a custom logger with a colorized stream handler. """
    return LoggerWrapper(formatter=formatter, name=name)
...
def get_logger(name: str) -> LoggerWrapper:
    ...
    global logger
    if logger is None:
        logger = setup_custom_logger(formatter=CustomFormatter(), name=name)
    return logger
